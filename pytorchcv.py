...
# load_mnist 함수 기반 load_Fashion_mnis 함수 정의 
...
def plot_results(hist): # plot_results라는 함수를 정의하는데 hist라는 이름의 딕셔너리를 매개변수로 받는데 학습과 검증 과정의 정확도와 손실이 배열 형태로 저장되어 있음
    ...
    plt.plot(hist['test_acc'], label='Testing acc') # hist 딕셔너리에서 훈련 정확도(test_acc)를 추출하여 그래프로 그리는데 라벨을 'Test acc'로 지정
    ...
    plt.plot(hist['test_loss'], label='Test loss') # hist 딕셔너리에서 훈련 손실(test_loss)을 추출하여 그래프로 그리는데 라벨을 'Test loss'로 지정
    plt.legend() # 그래프에 범례를 추가
    #validation -> test 변경 요
